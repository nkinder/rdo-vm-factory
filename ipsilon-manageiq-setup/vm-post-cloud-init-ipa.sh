...
# Add a group that matches the ManageIQ super admins group and
# make an IPA test user a member.
echo $IPA_PASSWORD | kinit admin
echo $IPA_PASSWORD | ipa user-add --first test --last user --password tuser
ipa group-add EvmGroup-super_administrator
ipa group-add-member EvmGroup-super_administrator --users tuser
kdestroy
